...
**Geographically weighted regression (GWR)** is a spatial statistical technique that takes non-stationary variables into consideration (e.g., climate; demographic factors; physical environment characteristics) and models the local relationships between these independent variables and an outcome of interest (also known as dependent variable). In this hands-on exercise, we will learn to build [hedonic pricing](https://www.investopedia.com/terms/h/hedonicpricing.asp) models by using GWR methods. The dependent variable is the resale prices of condominium in 2015. The independent variables are divided into either structural and locational.
...
```{r}
mpsz = st_read(dsn = "C:/Jacobche/ISSS624/Hands-on_Ex/Hands-on_Ex4/data/geospatial", layer = "MP14_SUBZONE_WEB_PL")
```

The report above shows that the R object used to contain the imported `MP14_SUBZONE_WEB_PL` shapefile is called `mpsz` and it is a simple feature object. The geometry type is multipolygon. It is also important to note that `mpsz` simple feature object does not have EPSG information.

Another way to do it is that we can transform it to 3414 directly here using *pipe* and *st_transform()*.
...
```{r}
condo_resale = read_csv("C:/Jacobche/ISSS624/Hands-on_Ex/Hands-on_Ex4/data/aspatial/Condo_resale_2015.csv")
```
...
In this section, we will learn to draw a small multiple histograms (also known as trellis plot) by using *ggarrange()* of [**ggpubr**](https://cran.r-project.org/web/packages/ggpubr/) package.
...
*tmap_options(check.and.fix = TRUE)* is added to resolve the invalid polygon issue.
...
In this section, we will learn to build hedonic pricing models for condominium resale units using [*lm()*](https://www.rdocumentation.org/packages/stats/versions/3.5.2/topics/lm) of base R.
...
Since p-value is much smaller than 0.0001, we will reject the null hypothesis that the mean is a good estimator of `SELLING_PRICE`. This will allow us to infer that simple linear regression model above is a good estimator of `SELLING_PRICE`.
...
Correlation matrix is commonly used to visualise the relationships between the independent variables. Beside the *pairs()* of R, there are many packages supporting the display of a correlation matrix. In this section, the [**corrplot**](https://cran.r-project.org/web/packages/corrplot/vignettes/corrplot-intro.html) package will be used.
...
Note that we should not use the spacial data frame here (ie `condo_resale.sf`).
...
Note that for the lm model, we can use the spacial data frame (ie `condo_resale.sf`).

With Adjusted R-squared being 0.6474, the model is able to explain close to 65% of the observations in dependable variable.
...
-   *ols_regress()* gives us a more tidy report as shown above.

-   With Adj. R-Squared being 0.647, the model is able to explain close to 65% of the observations in dependable variable.

-   All parameter estimates are significant with *Sig* less than 0.05.

-   Beta value refers to the extent of changes in dependent variable when the independent variable increases by one unit, with all other independent variables held constant. For example, if we hold all else constant, one unit increase in `AREA_SQM` will increase the retail price by 12777.523. Similarly if we hold all else constant, one unit increase in `AGE` will decrease the retail price by 24687.739. Also, if we hold all else constant, and if it is `FREEHOLD` the retail price will increase by 350599.812.
...
The result shows that the recommended bandwidth is 971.3405 metres, as seen in the last row of output. It is in metres since we are using **svy21**.
...
*longlat = FALSE* since our input data is projected already, so no conversion is needed.
...
The report shows that the adjusted r-square of the gwr is 0.8430 which is significantly better than the global multiple linear regression model of 0.6472. Additionally, the AICc of the gwr is 41632.36 which is smaller and thus better than the global multiple linear regression model of 42967.14.
...
The report shows that the adjusted r-square of the gwr is 0.8561 which is significantly better than the global multiple linear regression model of 0.6472. Additionally, the AICc of the gwr is 41982.22 which is smaller and thus better than the global multiple linear regression model of 42967.14.
...
Note that *tm_view(set.zoom.limits = c(11,14))* is used to control the zoom limits to assist the ease of use.
...
Note that *sync = TRUE* is added to add both interactive plots to move in tandem.
